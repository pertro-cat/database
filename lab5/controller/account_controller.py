...
def add_user():
    ...
    # Дозволені імена користувачів
    allowed_usernames = ['svitlana', 'petro', 'olha', 'taras']
    if data['username'] not in allowed_usernames:
        return jsonify({'message': f"Invalid username. Allowed usernames are: {', '.join(allowed_usernames)}"}), 400
